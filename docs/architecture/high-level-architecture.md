...
2.  A per-thread context is created, including an immutable configuration snapshot (via Owner) and instances of required services/drivers (via dedicated factories).
...
- **Factory Pattern:** Provides a simple, clear mechanism for creating complex objects like services or drivers without the overhead of a DI framework. This approach favors explicit creation over implicit injection, aligning with KISS and YAGNI principles.
