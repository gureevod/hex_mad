...
## Tasks / Subtasks
- [ ] Task 1: Enhance `BaseApiService` for Better Imperative Usage
  - [ ] Subtask 1.1: Add a protected `getRequestSpecification()` method in `BaseApiService` that returns the configured `RequestSpecification`.
  - [ ] Subtask 1.2: Ensure the existing `newRequest()` method continues to work for creating new requests.
  - [ ] Subtask 1.3: Add helper methods for common operations (e.g., `executeGet()`, `executePost()`, etc.).

- [ ] Task 2: Update `ApiServiceFactory` to Support Escape Hatches
  - [ ] Subtask 2.1: Add a static method `getRequestExecutor()` to retrieve a pre-configured `RequestExecutor`.
  - [ ] Subtask 2.2: Add a static method `getRequestSpecification()` to retrieve a pre-configured `RequestSpecification`.
  - [ ] Subtask 2.3: Ensure these methods use the default `ApiConfig` or accept a custom config parameter.

- [ ] Task 3: Create Pure Imperative Service Example
  - [ ] Subtask 3.1: Create `UserApiImperativeService.java` in `hex-project-samples` that extends `BaseApiService`.
  - [ ] Subtask 3.2: Implement all CRUD operations using direct RestAssured calls.
  - [ ] Subtask 3.3: Add complex methods like batch operations and file uploads.
  - [ ] Subtask 3.4: Ensure proper logging and error handling.

- [ ] Task 4: Create Hybrid Service Example
  - [ ] Subtask 4.1: Create `UserApiHybridService.java` that extends `BaseApiService` and implements `UserApi` interface.
  - [ ] Subtask 4.2: Implement declarative methods from the interface using delegation to declarative proxy.
  - [ ] Subtask 4.3: Add imperative methods for complex scenarios (e.g., dynamic query building, file upload with progress).
  - [ ] Subtask 4.4: Demonstrate method overriding where imperative implementation is preferred.
...
### File List
**To be created:**
- `hex-core-api/src/main/java/com/company/hex/api/service/BaseApiService.java` (enhance existing)
- `hex-core-api/src/main/java/com/company/hex/api/service/ApiServiceFactory.java` (enhance existing)
- `hex-project-samples/src/main/java/com/company/hex/project/api/services/UserApiImperativeService.java` (new)
- `hex-project-samples/src/main/java/com/company/hex/project/api/services/UserApiHybridService.java` (new)
- `hex-project-samples/src/test/java/com/company/hex/project/tests/api/UserApiImperativeTest.java` (new)
- `hex-project-samples/src/test/java/com/company/hex/project/tests/api/UserApiHybridTest.java` (new)
- `docs/HYBRID_API_GUIDE.md` (new)
